...
If the npm registry is temporarily unavailable, the CLI keeps enforcing the last successfully read
policy. A machine with no cached policy fails open so a registry outage cannot disable every command.
