...
const zh = {
  ...
  'db.duplicateTimestamps': '待执行迁移中存在重复时间戳，未执行任何迁移；时间戳用于确定回放顺序，必须唯一',
  'db.duplicateTimestamp': '重复时间戳 {timestamp}:',
...
const en: Record<MessageKey, string> = {
  ...
  'db.duplicateTimestamps': 'Pending migrations contain duplicate timestamps, so no migration was executed; timestamps determine replay order and must be unique',
  'db.duplicateTimestamp': 'Duplicate timestamp {timestamp}:',
