...
const zh = {
  ...
  'pull.gitBranchReady': 'Git 分支检查: 当前 {current}，配置推送分支 {configured}',
  'pull.gitBranchMismatch': '当前 Git 分支为 {current}，pull 会继续，但 sxq push 默认仅允许配置分支 {configured}',
  'pull.failed': '拉取失败',
  ...
  'push.gitOperation': 'Git 正在执行 {operation}，为避免推送中间状态，请完成或中止该操作后重试',
  'push.gitInspectFailed': '检测到 Git 工作树，但无法读取其分支或 HEAD；为避免绕过推送保护，本次 push 已中断',
  'push.gitValidated': 'Git 推送检查通过: 当前分支 {current}，配置分支 {configured}',
  'push.gitDetached': '当前处于 detached HEAD，不能确定推送分支；如确认要忽略分支限制，请使用 sxq push -f',
  'push.gitWrongBranch': '当前 Git 分支为 {current}，仅允许从配置分支 {configured} 推送；可用 sxq config set push-branch <branch> 修改，或用 sxq push -f 忽略分支限制',
  'push.gitBranchForced': '已使用 -f 忽略分支限制: 当前 {current}，配置 {configured}',
  'push.gitRootChanged': '.sxq 清单来自另一个 Git 工作树；请确认当前目录并先执行 sxq pull 更新本地清单',
  'push.gitManifestBranchChanged': '.sxq 清单最近在分支 {previous} 更新，当前分支为 {current}；请先在当前分支执行 sxq pull，或用 sxq push -f 忽略分支限制',
  'push.gitManifestBranchForced': '已使用 -f 忽略清单分支变化: {previous} → {current}',
  'push.gitHistoryChanged': '当前 Git 历史不是上次同步 HEAD 的后续，可能发生了 reset、rebase 或历史改写；请先执行 sxq pull 并检查工作树',
...
const en: Record<MessageKey, string> = {
  ...
  'pull.gitBranchReady': 'Git branch check: current {current}, configured push branch {configured}',
  'pull.gitBranchMismatch': 'Current Git branch is {current}. Pull will continue, but sxq push defaults to configured branch {configured}',
  ...
  'push.gitOperation': 'Git is currently running {operation}. Complete or abort it before pushing an intermediate worktree state',
  'push.gitInspectFailed': 'A Git worktree was detected, but its branch or HEAD could not be read. Push was aborted to avoid bypassing the Git safety checks',
  'push.gitValidated': 'Git push check passed: current branch {current}, configured branch {configured}',
  'push.gitDetached': 'The repository is at a detached HEAD, so the push branch cannot be determined. Use sxq push -f to ignore the branch restriction',
  'push.gitWrongBranch': 'Current Git branch is {current}; pushes are only allowed from configured branch {configured}. Run sxq config set push-branch <branch> to change it, or sxq push -f to ignore the branch restriction',
  'push.gitBranchForced': 'Ignoring the branch restriction with -f: current {current}, configured {configured}',
  'push.gitRootChanged': 'The .sxq manifest belongs to a different Git worktree. Verify the current directory and run sxq pull first to refresh the local manifest',
  'push.gitManifestBranchChanged': 'The .sxq manifest was last updated on branch {previous}; current branch is {current}. Run sxq pull on the current branch first, or use sxq push -f to ignore the branch restriction',
  'push.gitManifestBranchForced': 'Ignoring the manifest branch change with -f: {previous} -> {current}',
  'push.gitHistoryChanged': 'Current Git history does not descend from the last synced HEAD; reset, rebase, or another history rewrite may have occurred. Run sxq pull and inspect the worktree first',
  ...
  'cmd.pull': 'Pull remote code to local (Git projects show the configured push branch)',
  ...
  'cmd.pushForce': 'Ignore configured and manifest branch restrictions in a Git project',
